...
function OffRampModal({
  ...
  const banks = [
    ...
    { value: 'Techcombank', label: t('components.offramp.banks.techcombank', 'Techcombank (TCEX Thi diem)') },
    { value: 'Vietcombank', label: t('components.offramp.banks.vietcombank', 'Vietcombank') },
    { value: 'MB Bank', label: t('components.offramp.banks.mb', 'MB Bank') },
    { value: 'ACB', label: t('components.offramp.banks.acb', 'ACB') }
  ];
    ...
    <AnimatePresence>
      {isOpen && order && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <motion.button
            type="button"
            aria-label={t('components.offramp.close_aria', 'Dong modal rut tien')}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={handleClose}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal-700">
                  {t('components.offramp.tag', 'Off-ramp')}
                </p>
                <h2 className="mt-1 text-xl font-semibold text-slate-950">{t('components.offramp.title', 'Rut USDC ve ngan hang')}</h2>
              </div>
              <button
                type="button"
                onClick={handleClose}
                ...
                aria-label={t('components.offramp.close_aria', 'Dong')}
              >
                x
              </button>
            </div>
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_1.15fr]">
                <div className="space-y-4">
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    <p className="text-sm font-medium text-slate-500">{t('components.offramp.balance', 'So du kha dung')}</p>
                    <p className="mt-1 text-3xl font-semibold text-slate-950">{amount.toFixed(2)} USDC</p>
                    <div className="mt-4 space-y-2 text-sm">
                      <div className="flex justify-between gap-4">
                        <span className="text-slate-500">{t('components.offramp.exchange_rate', 'Ty gia gia dinh')}</span>
                        <span className="font-semibold text-slate-950">
                          {EXCHANGE_RATE.toLocaleString('vi-VN')} VND
                        </span>
                      </div>
                      <div className="flex justify-between gap-4">
                        <span className="text-slate-500">{t('components.offramp.receive', 'Thuc nhan')}</span>
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-700">{t('components.offramp.bank_name', 'Ngan hang')}</span>
                    <select
                      value={bankName}
                  ...
                  <label className="block">
                    <span className="text-sm font-semibold text-slate-700">{t('components.offramp.account_number', 'So tai khoan')}</span>
                    <input
                      value={bankAccount}
                      ...
                      placeholder={t('components.offramp.account_placeholder', 'Nhap so tai khoan')}
                    />
                  </label>
            ...
            {step === 'processing' && (
              <div className="py-16 text-center">
                ...
                <h3 className="mt-5 text-lg font-semibold text-slate-950">{t('components.offramp.processing_title', 'Dang xu ly lenh rut')}</h3>
                <p className="mt-2 text-sm text-slate-500">{t('components.offramp.processing_desc', 'ShopTalk dang gui yeu cau sang doi tac off-ramp.')}</p>
              </div>
            )}
            ...
            {step === 'success' && (
              <div className="py-10 text-center">
                ...
                <h3 className="mt-5 text-lg font-semibold text-slate-950">{t('components.offramp.success_title', 'Rut tien thanh cong')}</h3>
                <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                  {t('components.offramp.success_desc', '{{amount}} USDC da duoc quy doi thanh {{vnd}} VND va chuyen ve tai khoan {{account}} tai {{bank}}.', {
                    amount: amount.toFixed(2),
                    vnd: receivedVnd.toLocaleString('vi-VN'),
                    account: bankAccount,
                    bank: bankName
                  })}
                </p>
                ...
                >
                  {t('components.offramp.close_btn', 'Dong')}
                </button>
              </div>
            )}
          </motion.section>
