  ...
  const handleSend = async (event) => {
    ...
    try {
      ...
      try {
        ...
        // Fallback sang mock nếu API lỗi
      ...
      if (response.reply) {
        setMessages((current) => [
          ...current,
          { id: generateId(), role: 'assistant', content: response.reply }
        ]);
      }
    } catch (err) {
      console.error("❌ [Chat] Lỗi khi gửi tin nhắn:", err);
    } finally {
      setIsTyping(false);
    }
  };
...
<div className="flex items-center gap-3">
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-4">
        ...
        {isTyping && ( // Sử dụng isTyping từ nhánh main hoặc isLoading
          <div className="flex justify-start">
      ...
      {!isEscalated && (
        <div className="border-t border-[#243042] bg-[#0B0E14] p-4">
          <div className="mb-3 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            {suggestedPrompts.map((prompt) => (
              <button
                ...
                className="shrink-0 rounded-full border border-[#243042] bg-[#1C2533] px-3 py-1.5 text-xs font-semibold text-[#8F9CAE] transition hover:border-[#5B3FE0] hover:text-white"
              >
                {prompt}
              </button>
            ))}
          </div>
          {/* Form input của bạn sẽ tiếp tục ở đây */}
        </div>
      )}
